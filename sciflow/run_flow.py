...
__all__ = ['run_shell_cmd', 'make_shell_cmd', 'check_call_flow', 'check_call_flows', 'flow_task', 'run_flow_async',
           'run_flows_async', 'iter_param_grid', 'sample_grid_space', 'search_flow_grid', 'sciflow_check_metaflows',
...
async def run_flows_async(
    ...
    for flow_file_name in flow_file_names:
        ...
        if flow_file_name.startswith("_sciflow"):
            continue
        if flow_file_name.endswith(".py"):
            task = loop.create_task(flow_task(Path(flows_dir, flow_file_name), flow_provider, flow_command, params))
...
def search_flow_grid(flow_nb_path, param_grid, flow_provider="metaflow", total_tasks=None, n_conc_tasks=None, local_mode=True):
    ...
    task_batches = list(zip(sample_space, sample_space[1:]))[::n_conc_tasks]
    return task_batches

    #futures = search_batches(task_batches)
    #return futures
