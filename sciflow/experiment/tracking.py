...
__all__ = ['save_json', 'tracking_started', 'merge_step_entries', 'tracking_interrupted', 'tracking_failed',
           'tracking_completed', 'StepTracker']
...
def tracking_started(
    s3_res, bucket_name, flow_base_key, flow_run_id, steps, params=None
):
    # Create run-entry
    # Write run.json
...
def merge_step_entries(s3_res, bucket_name, run_entry_key, run_entry):
    all_hosts = {}
    captured_out = ""
    step_entries = {}
    for step in steps:
        step_entry = load_json(s3_res, bucket_name, s3_join(run_entry_key, f"step_{step}.json"))
...
def _tracking_event(
    s3_res, bucket_name, flow_base_key, flow_run_id, event_status, steps, except_info=None
):
    ...
    run_entry = merge_step_entries(s3_res, bucket_name, run_entry_key, run_entry)
    save_json(s3_res, bucket_name, run_entry_key, "run.json", run_entry)

# Cell


def tracking_interrupted(s3_res, bucket_name, flow_base_key, flow_run_id, steps):
    _tracking_event(s3_res, bucket_name, flow_base_key, flow_run_id, "INTERRUPTED", steps)
    print(f"Flow tracking interrupted: {flow_run_id}")

# Cell


def tracking_failed(s3_res, bucket_name, flow_base_key, flow_run_id, except_info, steps):
    _tracking_event(
        s3_res, bucket_name, flow_base_key, flow_run_id, "FAILED", steps, except_info
    )
...
def tracking_completed(s3_res, bucket_name, flow_base_key, flow_run_id, steps):
    _tracking_event(s3_res, bucket_name, flow_base_key, flow_run_id, "COMPLETED", steps)
