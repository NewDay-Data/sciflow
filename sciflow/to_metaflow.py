...
__all__ = ['rename_steps_for_metaflow', 'nb_to_metaflow', 'write_module_to_file', 'write_params', 'format_arg',
           'write_steps', 'write_track_capture', 'write_track_internal_helper', 'generate_flows', 'sciflow_metaflow']
...
def write_params(flow_file, param_meta, ind, track_experiment):
    for param in param_meta.keys():
        if param_meta[param].is_scalar:
            flow_file.write(
                f"{ind}{param} = Parameter('{param}', default={param})\n"
            )
    ...
    if track_experiment:
        ...
        flow_file.write(f"{ind}flow_base_key = Parameter('flow_base_key', required=True)\n")
...
def write_steps(
    flow_file, steps, orig_step_names, param_meta, ind, track_experiment
):
    for i, step in enumerate(steps):
        ...
        if i < len(orig_step_names):
            flow_step_args = ""
            if len(step.args) > 0:
                flow_step_args = ", ".join([format_arg(a, param_meta) for a in step.args.split(",")])

            step_func_call_text = f"{orig_step_names[i]}({flow_step_args})"

            if track_experiment:
                write_track_internal_helper(flow_file, ind, step, orig_step_names[i], step_func_call_text)
            ...
            if i < len(orig_step_names):
                ...
                if not step.has_return:
                    ...
                    flow_file.write(
                        f"{ind}{ind}results = {step_func_call_text}\n"
                    )
                    write_track_capture(flow_file, ind, 2)
            if i < len(steps):
                next_step = "end" if i == len(steps) - 1 else steps[i + 1].name
                flow_file.write(
                    f"{ind}{ind}self.next(self.{next_step})\n"
                )
            flow_file.write("\n")
...
def write_track_capture(flow_file, ind, num_indents):
    ...
    flow_file.write(f"{base_ind}{ind}{ind}self.__dict__[key] = self.__dict__[key] + results[key]\n")
...
def write_track_internal_helper(flow_file, ind, step, orig_step_name, step_func_call_text):
    flow_file.write(f"{ind}def _{step.name}(self):\n")
    if step.docstring:
        flow_file.write(f"{indent_multiline(step.docstring, 2)}\n")
    flow_file.write(f"{ind}{ind}tracker = None\n")
    flow_file.write(f"{ind}{ind}try:\n")
    flow_file.write(f"{ind}{ind}{ind}tracker = StepTracker(self.bucket_name, self.flow_base_key, self.flow_run_id, \"{orig_step_name}\")\n")
    flow_file.write(f"{ind}{ind}{ind}with tempfile.TemporaryDirectory() as temp_dir:\n")
    flow_file.write(f"{ind}{ind}{ind}{ind}with tracker.capture_out() as tracker._output_file:\n")
    ...
    flow_file.write(f"{ind}{ind}{ind}{ind}exc_type, exc_value, trace = sys.exc_info()\n")
    flow_file.write(f"{ind}{ind}{ind}{ind}except_info = {{\"exc_type\": exc_type, \"exc_value\": exc_value, \"trace\": trace}}\n")
    flow_file.write(f"{ind}{ind}{ind}{ind}tracker.completed(status=\"FAILED\", except_info=except_info)\n")
