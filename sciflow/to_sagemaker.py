...
__all__ = ['nb_to_sagemaker_pipeline', 'is_train_step', 'is_processing_step', 'write_pipeline_to_files',
           'write_track_flow', 'write_params', 'upload_directory', 'extract_step_vars', 'format_job_arguments',
           'format_hyperparams', 'format_arg', 'write_steps', 'write_track_capture', 'get_return_var_names',
           'format_args', 'generate_sagemaker_modules']
...
def write_pipeline_to_files(
    ...
    with open(flow_path, "w") as flow_file:
        ...
        flow_reqs_path = Path(
            lib_path(), get_config().flows_path, "sagemaker", "requirements.txt"
        )
        ...
        flow_file.write(
            f'{ind}{ind}self.lib_code_key = f"{{self.s3_prefix}}/code/{get_config().lib_name}"\n'
        )
        flow_file.write(
            ...
            if len(step_vars) > 0:
                ...
                if 'step_proc_vars' in step_vars and len(step_vars['step_proc_vars']) > 0:
                    ...
                    for training_input in step_vars['step_proc_vars']:
                        flow_file.write(f'{ind}{ind}{ind}{ind}"{training_input}": TrainingInput(\n')
                        # TODO store content type mapping
                        flow_file.write(f"{ind}{ind}{ind}{ind}{ind}s3_data=self.{outputs[training_input]}, content_type=\"text/csv\"\n")
        ...
        with open(sm_module_path, "w") as sm_module_file:
            ...
            if is_processing_step(step):
                ...
                if (
                    ...
                    for step_train_var in steps_vars[step.name]["step_train_vars"]:
                        sm_module_file.write(
                            f'{ind}{step_train_var} = load_result("/opt/ml/processing/input", "{step_train_var}")\n'
                        )
            elif is_train_step(step):
                if (
                    "step_proc_vars" in steps_vars[step.name]
                    ...
                    for step_proc_var in steps_vars[step.name]["step_proc_vars"]:
                        sm_module_file.write(
                            f'{ind}{step_proc_var} = load_result("/opt/ml/input/data/documents", "{step_proc_var}")\n'
                        )
