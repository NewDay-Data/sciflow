...
def evaluate(model):
    ...
    # TODO figure artifacts out for sagemaker
    artifacts = ["dataframe_artifact.csv"]
...
def main(documents, workers):
    ...
    pd.DataFrame({'docs': [1,2,3,4], 'hello': [5,5,6,7]}).to_csv("/opt/ml/processing/word_summaries/word_summaries.csv")
    pd.DataFrame({'docs': [1,2,3,4], 'hello': [5,5,6,7]}).to_csv("/opt/ml/processing/artifacts/artifacts.csv")
    pd.DataFrame({'docs': [1,2,3,4], 'hello': [5,5,6,7]}).to_csv("/opt/ml/processing/metrics/metrics.csv")
